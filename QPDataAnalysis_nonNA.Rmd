...
# prop <- function(df,theme){
#   theme <- subset(df, select = c(Root,theme)) # create temporary dataset with one theme and list of roots
#   theme <- na.omit(theme) # delete rows with NA
#   count_theme <- theme %>% dplyr::count(Root) # count repetitions of roots (it will be 6 when there is no NA, less if there were NAs that got deleted)
#   theme <- aggregate(. ~ Root, theme, sum) # sum over column of theme for each root
#   theme <- mutate(theme,count=count_theme$n) # add count of roots to this dataset
#   theme <- mutate(theme,prop_theme=theme/count) # create column for proportion = sum of theme / count of root
# }
# 
# suum <- prop(dataset,suum)
...
```
```{r}
# Color according to Animate 
new <- left_join(solution,both)
fig <- plot_ly(new, x = ~Dimension.1, y = ~Dimension.2, z = ~Dimension.3, mode = 'text', text = ~rootnames,
               color= ~Animate) #, colors = c('#BF382A', '#0C4B8E')
fig <- fig %>% add_markers()
fig <- fig %>% layout(scene = list(xaxis = list(title = 'Dim 1'),
                     yaxis = list(title = 'Dim 2'),
                     zaxis = list(title = 'Dim 3')))
...
```{r}
# Color according to Blockage.of.motion
fig <- plot_ly(new, x = ~Dimension.1, y = ~Dimension.2, z = ~Dimension.3, mode = 'text', text = ~rootnames,
               color= ~Blockage.of.motion)
fig <- fig %>% add_markers()
fig <- fig %>% layout(scene = list(xaxis = list(title = 'Dim 1'),
                     yaxis = list(title = 'Dim 2'),
                     zaxis = list(title = 'Dim 3')))
...
```{r}
# Color according to Potential.energy
fig <- plot_ly(new, x = ~Dimension.1, y = ~Dimension.2, z = ~Dimension.3, mode = 'text', text = ~rootnames,
...
```{r}
# Color according to Orientation.in.vertical
fig <- plot_ly(new, x = ~Dimension.1, y = ~Dimension.2, z = ~Dimension.3, mode = 'text', text = ~rootnames,
               color= ~Orientation.in.vertical)
fig <- fig %>% add_markers()
fig <- fig %>% layout(scene = list(xaxis = list(title = 'Dim 1'),
                     yaxis = list(title = 'Dim 2'),
                     zaxis = list(title = 'Dim 3')))
